...
seed <- seed_ra %>% 
  ...
  mutate(clade = ifelse(species %in% c("agsp", "caly", "dece", "juar", "juba", "juen", "jute", "poasp"), "Select Graminoid",
                 ifelse(species %in% c("agsp", "caly", "dece", "elpar", "isce", "juar", "juba", "juen", "jute", "poasp"), "All Gramindoid", "Forb"))) %>% 
...
```{r boxplots of native vs exotic in veg vs seed, by fngp}
...
cover_graminoid <- obs_fngp %>%
  filter(fngp2 == "Perennial graminoid")
...
```{r boxplots sp of interest in veg vs seed, by fngp}
...
want to do this https://stackoverflow.com/questions/45611725/multiple-rows-in-facet-grid 

#ggsave("fig-boxplot_soi_seed-veg.jpg", soi, width = 15, height = 10)


```
Vegetation

```{r veg summary: native, exotic, cultural, sedge, richness}
veg.sum <- mie %>% 
  filter(!species == "bare") %>% 
  filter(cover != 0) %>% 
...
# redoing veg.sum w/ ra_join (instead of mie), adding additional levels of group_by fngp2/endemic. Compare to running analyses w veg.sum



```

```{r veg boxplots, faceted; older iteration/may be obsolete}
...
#                             labels = c("Culturally Significant Species", "Native Species", "Exotic Species", "Total Species"))
...
#                             labels = c("Culturally Significant Species", "Native Species", "Exotic Species", "Carex lyngbyei", "Select Graminoids", "Total Species"))
# 
# pd <- position_dodge(0.75)
# 
# rich_box <- veg_box %>% 
#   ggplot(aes(x = disturbance, y = rich_val, fill = richness)) +
#   geom_boxplot() + # aes(fill = richness), outlier.shape = NA
#   #geom_point(position = position_dodge(width = 1), color = 'gray60', size = 2) +
#   #geom_jitter(alpha = 0.1, width = 0.3) +   # , position = pd
#   theme_classic() +
#   facet_grid(. ~ estuary, scales = "free", labeller = labeller(estuary = facet_labels)) +
#   labs(x = "Disturbance condition", y = "Species Richness", fill = "Richness") +
...
#   theme_classic() +
#   facet_grid(. ~ estuary, scales = "free", labeller = labeller(estuary = facet_labels)) +
#   labs(x = "Disturbance condition", y = "Species Richness", fill = "Above-ground Cover") +
#   #scale_y_continuous(breaks = seq(0, 12, by = 2)) +
#   theme(text = element_text(size = 12), 
#         panel.spacing.x = unit(1.25, "lines"), 
#         axis.text.x = element_text(angle = 25, hjust = 1)) +
...
```{r veg boxplot, unfaceted - may be obsolete; see next chunk}
...
```{r veg vs seed boxplots, by fngp natve v exotic}
...
cover_graminoid %>% 
  ggplot(aes(x = endemic, y = cover, fill = obs, shape = obs)) +
  geom_boxplot() +
  facet_grid(estuary~disturbance) +
  geom_jitter(width = 0.1, alpha = 0.2) +
   theme_classic()
...
# plot the glm output over the boxplot https://stackoverflow.com/questions/46364748/plot-visreg-over-an-boxplot-glm-with-binominal-predictor
# library(visreg)
# gram_fit <- visreg(graminoid_glm, scale = "response", plot = FALSE)
# visreg(graminoid_glm, "disturbance")
...
seed.sum <- seed %>% 
  #filter(cover != 0) %>% 
  group_by(id, disturbance, estuary) %>% 
  dplyr::summarize(seed_native_rich = length(endemic[endemic == "Native"]), 
            seed_exotic_rich = length(endemic[endemic == "Invasive"]), 
            seed_total_rich = length(species[unique(species)]), 
            seed_cultural_rich = length(species[cultural == "cultural"]), 
            native_m2 = sum(abund.m2[endemic == "Native"]), 
            exotic_m2 = sum(abund.m2[endemic == "Invasive"]), 
            cultural_m2 = sum(abund.m2[cultural == "cultural"]), 
            select_graminoid_m2 = sum(abund.m2[clade == "Select Graminoid"]),
            sedge_m2 = sum(abund.m2[species == "caly"]), 
            total_m2 = sum(abund.m2)) %>% 
    na.omit() 
...
```{r seed response boxplots}

seed.sum$estuary = factor(seed.sum$estuary,
                     levels = c("nre", "lqre"),
                     labels = c("Nanaimo", "Little Qualicum"))
seed.sum$disturbance = factor(seed.sum$disturbance,
                            levels = c("grubbed", "excl1", "excl10", "reference"),
                            labels = c("Grubbed", "Heavily Grazed, \nExclosed 1 year", "Heavily Grazed, \nExclosed 10 years", "Undisturbed")) 



seed_graminoid <- seed.sum %>% 
  ggplot(aes(x = disturbance, y = select_graminoid_m2, fill = estuary, shape = estuary)) +
  geom_boxplot() + geom_jitter(width = 0.1, alpha = 0.2) +
  theme_classic() +
  labs(x = "Disturbance condition", y = "Graminoid abundance, \nexcluding Eleocharis parvula (m2)", fill = "Estuary") +
   theme_classic()+
   theme(text = element_text(size = 12), 
           panel.spacing.x = unit(1.25, "lines"), 
           axis.text.x = element_text(angle = 25, hjust = 1)) +
...
seed_sedge <- seed.sum %>%
  ggplot(aes(x = disturbance, y = sedge_m2, fill = estuary, shape = estuary)) +
  geom_boxplot() + geom_jitter(width = 0.1, alpha = 0.2) +
  #facet_wrap(~estuary, scales = "free") +
  theme_classic() +
  labs(x = "Disturbance condition", y = "Sedge abundance (m2)", fill = "Estuary") +
   theme_classic()+
   theme(text = element_text(size = 12),
           panel.spacing.x = unit(1.25, "lines"),
           axis.text.x = element_text(angle = 25, hjust = 1)) +
...
seed_total <- seed.sum %>%
  ggplot(aes(x = disturbance, y = total_m2, fill = estuary, shape = estuary)) +
  geom_boxplot() + geom_jitter(width = 0.1, alpha = 0.2) +
  theme_classic() +
  labs(x = "Disturbance condition", y = "Total abundance (m2)", fill = "Estuary") +
   theme_classic()+
   theme(text = element_text(size = 12),
           panel.spacing.x = unit(1.25, "lines"),
           axis.text.x = element_text(angle = 25, hjust = 1)) +
...
seed_exotic <- seed.sum %>% 
  ggplot(aes(x = disturbance, y = exotic_m2, fill = estuary, shape = estuary)) +
  geom_boxplot() + geom_jitter(width = 0.1, alpha = 0.2) +
  theme_classic() +
  labs(x = "Disturbance condition", y = "Exotic seed abundance (m2)", fill = "Estuary") +
   theme_classic()+
   theme(text = element_text(size = 12), 
           panel.spacing.x = unit(1.25, "lines"), 
           axis.text.x = element_text(angle = 25, hjust = 1)) +
...
seed_cultural <- seed.sum %>% 
  ggplot(aes(x = disturbance, y = cultural_m2, fill = estuary, shape = estuary)) +
  geom_boxplot() + geom_jitter(width = 0.1, alpha = 0.2) +
  theme_classic() +
  labs(x = "Disturbance condition", y = "Culturally significant species abundance (m2)", fill = "Estuary") +
   theme_classic()+
   theme(text = element_text(size = 12), 
           panel.spacing.x = unit(1.25, "lines"), 
           axis.text.x = element_text(angle = 25, hjust = 1)) +
...
seed_rich_total <- seed.sum %>% 
  ggplot(aes(x = disturbance, y = seed_total_rich, fill = estuary, shape = estuary)) +
  geom_boxplot() + geom_jitter(width = 0.1, alpha = 0.2) +
  theme_classic() +
  labs(x = "Disturbance condition", y = "Total Richness", fill = "Estuary") +
   theme_classic()+
...
seed_rich_exotic <- seed.sum %>% 
  ggplot(aes(x = disturbance, y = seed_exotic_rich, fill = estuary, shape = estuary)) +
  geom_boxplot() + geom_jitter(width = 0.1, alpha = 0.2) +
  theme_classic() +
  labs(x = "Disturbance condition", y = "Exotic Richness", fill = "Estuary") +
   theme_classic()+
...
seed_rich_cultural <- seed.sum %>% 
  ggplot(aes(x = disturbance, y = seed_cultural_rich, fill = estuary, shape = estuary)) +
  geom_boxplot() + geom_jitter(width = 0.1, alpha = 0.2) +
  theme_classic() +
  labs(x = "Disturbance condition", y = "Culturally significant species richness", fill = "Estuary") +
   theme_classic()+
...
seed_rich_panel <- ggarrange(seed_rich_total, seed_rich_exotic, seed_rich_cultural,
                       common.legend = TRUE, legend = "right",
                       ncol = 1, nrow = 3) 
                       #widths = c(2, 2)) #%>% 
             #annotate_figure(bottom = text_grob("text here", hjust = 1, x = 1,  size = 20))
...
seed_abund_panel <- ggarrange(seed_total, seed_graminoid, seed_sedge, seed_exotic, seed_cultural,
                       common.legend = TRUE, legend = "right",
                       ncol = 2, nrow = 4)
seed_abund_panel
...
seed.ra <- seed %>% 
  select(estuary, disturbance, id, ct, species, endemic, fngp2, cultural, clade) %>% 
  group_by(id, species) %>%
  dplyr::summarize(sum.seed = sum(ct),
                   rel.abund = (ct/sum.seed)*100,
                   species = species)

  #aggregate(rel.abund~species, FUN=sum) #https://stackoverflow.com/questions/10180132/consolidate-duplicate-rows
  #ungroup()
#View(seed.ra)
...
seasonal.ra <- seed.ra %>% 
  select(estuary, disturbance, id, species, rel.abund) %>% 
...
# select only species
seasonal.sp = seasonal.ra[,4:ncol(seasonal.ra)]
...
set.seed(123)
nmds = metaMDS(m.com, distance = "euclidean")
...
data.scores$endemic = pa.join$endemic
...
pa.nmds = ggplot(data.scores, aes(x = NMDS1, y = NMDS2)) + 
    ...
    geom_point(size = 6, aes( colour = obs, shape = endemic))+ 
    facet_grid(estuary ~ disturbance, labeller = labeller(disturbance = disturb_labels)) + 
    ...
    panel.background = element_blank(), panel.border = element_rect(colour = "black", fill = NA, size = 1.2),
    legend.key=element_blank()) + 
    labs(x = "NMDS1", colour = "Source", y = "NMDS2", shape = "Status", caption = "Dissimilarity of species presence/absence using Euclidean distance, stress = 0.013")  + 
    theme(strip.text.x = element_text(size = 14, face = "bold")) +
    scale_colour_brewer(palette = "Dark2", limits = c("veg", "seed"), labels = c("Above ground vegetation", "Surface seed bank")) +
    scale_shape_manual(values = c(15, 17), limits = c("Native", "Invasive"), labels = c("Native", "Exotic")) 
...
compare.ra.nmds.nre = ggplot(ds.compare.ra.nre, aes(x = NMDS1, y = NMDS2)) + 
    geom_point(size = 4, aes(colour = obs)) + 
    ...
    labs(x = "NMDS1", colour = "Source", shape = "Endemic status", y = "NMDS2", title = "Nanaimo", caption = "stress = 0.0834")  + 
    scale_color_manual(limits = c("veg", "seed"), labels = c("Above ground vegetation", "Surface seed bank"), values = col) # +
    #scale_shape_manual(values = c(15, 17), limits = c("Native", "Invasive"), labels = c("Native", "Exotic")) 
...
compare.ra.nmds.lqre = ggplot(ds.compare.ra.lqre, aes(x = NMDS1, y = NMDS2)) + 
    ...
    legend.key=element_blank()) +
    facet_grid(. ~ disturbance, labeller = labeller(disturbance = disturb_labels)) + 
    labs(x = "NMDS1", colour = "Source", y = "NMDS2", title = "Little Qualicum", caption = "stress = 0.0898")  + 
    theme(strip.text.x = element_text(size = 16), plot.caption = element_text(size = 16)) +
    scale_color_manual(limits = c("veg", "seed"), labels = c("Above ground vegetation", "Surface seed bank"), values = col) #+
    #scale_shape_manual(values = c(15, 17), limits = c("Native", "Invasive"), labels = c("Native", "Exotic")) 
...
ggsave("fig-nmds-compare-ra-panel.jpg", compare.ra.nmds.panel, width = 15, height = 10)
...
```{r DELETE - OBSOLETE CALCS relabund in all seed & veg}
...
# seed.select <- seed %>%
#   select(-c(life, season, root, abund.m2)) %>%
#   mutate(obs = "seed") %>%
#   unite(endemic.fngp2, c(endemic, fngp2), sep = " ", remove = FALSE)
# #View(seed.select)
# #View(seed)
...
# 
# seed_sum <- seed %>% 
#   select(estuary, disturbance, id, species, ct, abund.m2) %>% 
#   group_by(estuary, disturbance, id, species) %>% 
#   dplyr::summarize(sp_sum = sum(ct[species == species]), 
#                    sp_seed_m2 = sum(abund.m2[species == species])) %>% 
#   ungroup() %>% 
#   na.omit()
# #View(seed_sum)
# unique(seed_sum$id)
...
# seed_join <- seed_sum %>% 
#   left_join(seed) %>% 
#   select(estuary, disturbance, id, species, ct, abund.m2, sp_sum, sp_seed_m2) %>% 
#   group_by(id) %>%
#   dplyr::summarize(id_sum = sum(ct[id == id]),
#                    sp_seed_ra = ((sp_sum/id_sum)*100)) %>% 
#   ungroup()
# View(seed_join)
# 
# seed_ra <- seed_join %>% 
#   left_join(seed_sum) %>% 
#   left_join(seed) %>% 
#   select(estuary, disturbance, id, species, sp_sum, sp_seed_m2, sp_seed_ra) %>% 
#   group_by(estuary, disturbance, id, species)
# View(seed_ra)
