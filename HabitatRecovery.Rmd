...
```{r boxplots sp of interest, by obs & fngp}
...
```{r NMDS of relative abundance - grid estuary~disturbance}
...
```{r DIDN'T WORK; REVISIT - COULDN'T GET FACTORS AS MEANINGFUL CENTROIDS/VECTORS ON PLOT for nmds relabund, panel .~estuary, with disturbance & fngp as factors}
# tried using envfit to make vector & factor ecological coordinates per https://jkzorz.github.io/2020/04/04/NMDS-extras.html
...
ggplot(data = ds_nre, aes(x = NMDS1, y = NMDS2)) + 
     geom_point(data = ds_nre, aes(colour = fngp2, shape = obs), size = 3, alpha = 0.5) + 
