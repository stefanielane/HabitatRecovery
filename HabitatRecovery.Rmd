...
# raw.mie %>% pivot_longer(agre:trma, names_to = "species", values_to = "cover") 
mie <- raw.mie %>% pivot_longer(agre:bare, names_to = "species", values_to = "cover") %>%
  mutate(obs = "veg") %>% 
...
d.seed <- read.csv("seeds_UNCONFIRMED.csv")
head(d.seed)
# unique(d.seed[c("species")])
## popa, juba, agsp, spca, graminoid, elpar, sysu, coco, isce, sade, caly, glma, poasp, dece, cirsp, juar, forb, achmi, jubu, epci,	juen, trma, na, oxsp, forb1	, forb2	, atpa

seed <- d.seed %>% 
  filter(!species %in% c("graminoid", "na", "oxsp")) %>% 
  select(-c("id", "latin", "action")) %>% 
  mutate(obs = "seed") %>% 
  mutate(endemic = ifelse(species %in% c("agsp", "atpa", "coco", "cirsp", "jubu", "poasp"), "Invasive", "Native")) %>% 
  mutate(life = ifelse(species %in% c("atpa", "isce","jubu", "sade", "spca", "poasp", "forb", "forb1", "forb2"), "Annual", "Perennial")) %>% 
  mutate(root = ifelse(species %in% c("atpa", "cirsp", "coco", "dece", "isce", "jubu", "sade", "poasp", "forb", "forb1", "forb2"), "Fibrous", "Rhizomatous")) %>% 
  mutate(clade = ifelse(species %in% c("agsp", "caly", "dece", "elpar", "isce", "juar", "juba", "juen", "jubu", "poasp"), "Graminoid", "Forb")) %>% 
  mutate(fngp = ifelse(species %in% c("agsp", "caly", "elpar", "juar", "juba", "juen"), "Perennial graminoid,  rhizomatous roots",                  
                  ifelse(species %in% c("dece"), "Perennial graminoid,  fibrous roots",
                  ifelse(species %in% c("atpa", "sade", "spca", "forb", "forb1", "forb2"), "Annual forb", "Perennial forb")))) %>% 
   mutate(fngp2 = ifelse(species %in% c("agsp", "caly", "juar", "juen", "juba", "dece", "elpar"), "Perennial graminoid",
                  ifelse(species %in% c("isce", "jubu"), "Annual graminoid", 
                  ifelse(species %in% c("atpa", "sade", "spca", "forb", "forb1", "forb2"), "Annual forb", "Perennial forb")))) %>% 
  mutate(abund.m2 = count*125) %>%  # 125 multiplier estimates sample area of ~80 cm2 to fill 1 m2 (10,000 cm2) 
  group_by(estuary, disturbance, replicate, species) %>% 
  dplyr::rename(id = replicate) %>% 
...
#View(seed)
seed.select <- seed %>% 
  select(-c(life, root, clade, abund.m2)) %>% 
...
# View(mie.select)
mie.rel <- mie.select %>% 
  filter_at(vars("cover"), any_vars(. != 0)) %>% 
  group_by(estuary, disturbance, id) %>% 
  dplyr::summarize(sum.veg = sum(cover),
...
ra.join <- mie.ra %>% 
  ...
  select(-c("cover", "ct", "endemic","fngp","fngp2")) %>% 
  unite(endemic.fngp2, c(endemic, fngp2), sep = " ", remove = FALSE)
...
# if adding seeds: seed shape = 17, seed limit = "seed", seed label = "Surface Seed Bank",

```

``` {r surface seed bank trends}
...
seed.abund <- seed.abund.mean %>% 
  ...
  facet_grid(season ~ estuary, labeller = labeller(estuary = facet_labels, season = season_labels)) +
...
```{r nmds of vegetation and summer seed bank, facet wrap by estuary}
...
seed.pa <- seed %>% 
  ...
  mutate_all(~replace(., is.na(.), 0))  %>% 
  select(-c(season, life, root, clade, fngp, abund.m2))
...
pa.join <- mie.pa %>% 
  full_join(seed.pa, by = c("estuary", "endemic", "fngp2", "disturbance", "id", "obs", "agsp", "caly", "coco", "glma", "elpar", "juba", "sade", "spca", "sysu", "trma")) %>% 
  replace(is.na(.), 0) 

pa.join <- pa.join[which(rowSums(pa.join[,7:35]) > 0), ] # remove rows where all species columns = 0
#View(pa.join)
#ncol(pa.join) #35
#colnames(pa.join)
# which(is.na(pa.join))
