...
mie <- raw.mie %>% pivot_longer(agre:bare, names_to = "species", values_to = "cover") %>%
  ...
  mutate(life = ifelse(species %in% c("atpa", "pofo", "sade", "spca"), 
                              "Annual", ifelse(species %in% "bare", "Unvegetated", "Perennial"))) %>% 
...
seed <- d.seed %>% 
  ...
  mutate(fngp = ifelse(species %in% c("agsp", "caly", "elpar", "juar", "juba", "juen"), "Perennial graminoid,  rhizomatous roots",                  
   ...
   mutate(fngp2 = ifelse(species %in% c("agsp", "caly", "juar", "juen", "juba", "dece", "elpar"), "Perennial graminoid",
                  ...
                  ifelse(species %in% c("atpa", "sade", "spca", "forb", "forb1", "forb2"), "Annual forb", "Perennial forb")))) %>% 
   mutate(structure = ifelse(species %in% c("coco", "isce" , "glma", "elpar", "jubu", "popa", "sade", "spca"), "short", 
                    ifelse (species %in% c("achmi", "agsp", "atpa", "forb", "forb1", "forb2", "juar", "juba", "juen", "sysu", "trma"), "med", "tall"))) %>%  # short = < 50 cm; med = 50 - 1 m, tall = > 1 m, per eflora BC
...
ra.join <- mie.ra %>% 
  full_join(seed.ra, by = c("estuary", "disturbance", "id","species","obs","endemic.fngp2", "endemic","fngp", "fngp2","rel.abund", "season", "structure")) %>% 
  unite(endemic.fngp2, c(endemic, fngp2), sep = " ", remove = FALSE) %>% 
  ungroup() %>% 
   select(-c("ct", "endemic","fngp","fngp2"))
...
pa.join <- mie.pa %>% 
  full_join(seed.pa, by = c("estuary", "endemic", "fngp2", "disturbance", "id", "obs", "structure", "agsp", "caly", "coco", "glma", "elpar", "juba", "sade", "spca", "sysu", "trma")) %>% 
  replace(is.na(.), 0) 

pa.join <- pa.join[which(rowSums(pa.join[,8:30]) > 0), ] # remove rows where all species columns = 0
#View(pa.join)
#ncol(pa.join) #30
#colnames(pa.join)
# which(is.na(pa.join))
...
compare.ra.nmds.nre = ggplot(ds.compare.ra.nre, aes(x = NMDS1, y = NMDS2)) + 
    ...
    theme(strip.text.x = element_text(size = 16), plot.caption = element_text(size = 16)) +
    scale_x_continuous(breaks = seq(-0.75, 0.5, 0.5)) +
    labs(x = "NMDS1", colour = "Season", shape = "Life Stage", y = "NMDS2", title = "Nanaimo", caption = "stress = 0.121")  + 
    scale_color_manual(limits = c("summer", "fall", "spring"), labels = c("Summer 2021", "Fall 2021", "Spring 2022"), values = seasoncol) +
  scale_shape_manual(values = c(15, 17), limits = c("veg", "seed"), labels = c("Vegetation", "Surface Seed Bank"))
compare.ra.nmds.nre
...
compare.ra.nmds.lqre = ggplot(ds.compare.ra.lqre, aes(x = NMDS1, y = NMDS2)) + 
    ...
    legend.key=element_blank()) +
    facet_grid(. ~ disturbance, labeller = labeller(disturbance = disturb_labels)) +
    labs(x = "NMDS1", colour = "Season", shape = "Life Stage", y = "NMDS2", title = "Little Qualicum", caption = "stress = 0.157")  + 
    scale_x_continuous(breaks = seq(-0.75, 0.5, 0.5)) +
    theme(strip.text.x = element_text(size = 16), plot.caption = element_text(size = 16)) +
    scale_color_manual(limits = c("summer", "fall", "spring"), labels = c("Summer 2021", "Fall 2021", "Spring 2022"), values = seasoncol) +
  scale_shape_manual(values = c(15, 17), limits = c("veg", "seed"), labels = c("Vegetation", "Surface Seed Bank"))
