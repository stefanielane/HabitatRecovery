...
#fngp are finescale groups; fngp2 are coarser groupings
mie <- d_mie %>% pivot_longer(agre:bare, names_to = "species", values_to = "cover") %>%
  mutate(obs = "veg") %>% 
  mutate(endemic = ifelse(species %in% c("agre", "agsp", "atpa", "coco", "caca"), "Invasive",
                   ifelse(species %in% "bare", "NA", "Native"))) %>% 
  mutate(life = ifelse(species %in% c("atpa", "isce", "pofo", "sade", "spca"), 
                              "Annual", ifelse(species %in% "bare", "NA", "Perennial"))) %>% 
  mutate(root = ifelse(species %in% c("atpa", "daca", "pofo", "trwo"), "Taproot", 
                ifelse(species %in% c("coco", "dece", "isce", "sade"), "Fibrous",
                ifelse(species %in% "bare", "NA", "Rhizomatous")))) %>% 
  mutate(clade = ifelse(species %in% c("agre", "agsp", "caly", "dece", "disp", 
                                      "isce", "juba"), "Graminoid",
                ifelse(species %in% "bare", "NA", "Forb"))) %>% 
  mutate(fngp = ifelse(species %in% c("agre", "agsp", "caly", "disp", "juba"), "Perennial graminoid, rhizomatous roots",
                  ifelse(species %in% c("isce"), "Annual graminoid", 
  ...
  mutate(fngp2 = ifelse(species %in% c("agre", "agsp", "caly", "disp", "juba", "dece"), "Perennial graminoid",
                  ifelse(species %in% c("isce", "atpa", "pofo", "sade", "spca"), "Annual", 
                  ifelse(species %in% c("coco", "daca", "trwo", "trma", "glma", "popa", "sysu"), "Perennial forb", "Unvegetated"))))

mie$estuary <- factor(mie$estuary, levels = c("ere", "nre", "lqre"))
#View(mie)


d_seed <- read.csv("seeds.csv")


seed <- d_seed %>% 
  #filter(!replicate > 8) %>% 
  mutate(obs = "seed") %>% 
  mutate(endemic = ifelse(species %in% c("agsp", "coco"), "Invasive", "Native")) %>% 
  mutate(life = ifelse(species %in% c("isce","sade", "spca"), "Annual", "Perennial")) %>% 
  mutate(root = ifelse(species %in% c("coco", "dece", "isce", "sade"), "Fibrous", "Rhizomatous")) %>% 
  mutate(clade = ifelse(species %in% c("agsp", "caly", "dece", "graminoid_x", "juox", "schoenoplectus_unconfirmed"), "Graminoid", "Forb")) %>% 
  mutate(fngp = ifelse(species %in% c("agsp", "caly", "graminoid_x", "juox", "schoenoplectus_unconfirmed"), "Perennial graminoid,  rhizomatous roots",
                  ifelse(species %in% c("isce"), "Annual graminoid", 
                  ifelse(species %in% c("dece"), "Perennial graminoid,  fibrous roots",
                  ifelse(species %in% c("sade", "spca"), "Annual forb", "Perennial forb"))))) %>% 
   mutate(fngp2 = ifelse(species %in% c("agsp", "caly", "graminoid_x", "juox", "schoenoplectus_unconfirmed", "dece"), "Perennial graminoid",
                  ifelse(species %in% c("sade", "spca", "isce"), "Annual", "Perennial forb"))) %>% 
  mutate(abund.m2 = count*66.67)
#View(seed)
...
facet_labels <- c(ere = "Englishman", nre = "Nanaimo", lqre = "Little Qualicum")
...
We compare the recovery trajectory of exclosures that are 1 versus 10 years old and ask the questions 
1) How does CAGO grazing impact species and Plant Functional Groups (PFGs)?  
2) How do species and PFGs respond over time since grazing exclusion?  
3)  How do seed banks respond over time since grazing exclusion? 
4) Is seed bank composition similar to above-ground species composition?

We used mixed effects models to test time since grazing exclusion on plant species cover, seed bank richness, seed bank abundance, and abundance of native and non-native species following grazing exclusion across seasons. We used NMDS to illustrate seed bank dissimilarity from above-ground vegetation. 

```{r impact to PFGs}

mie$fngp <- factor(mie$fngp , levels = c("Annual forb", "Annual graminoid, fibrous roots", "Perennial forb, fibrous roots", "Perennial forb, rhizomatous roots",  "Perennial graminoid, fibrous roots", "Perennial graminoid, rhizomatous roots", "Unvegetated"))
...
veg_end <- mie %>% select(estuary, disturbance, elev, sal, endemic, fngp, cover) %>% 
...
veg_end %>%  
  ...
  geom_jitter(position = pd, 
             size = 7) +
  geom_errorbar(aes(ymin = veg_end.mean - veg_end.se, 
  ...
  scale_color_brewer(limits = c("Native", "Invasive", "NA"), labels = c("Native", "Invasive", "Bare Ground"), palette = "Set2")

```

```{r PFGs response over time}
...
seed.end.mean <- seed %>% 
  ...
  dplyr::summarize(seed.end.n = n(),
            seed.end.mean = mean(count), 
            seed.end.sd = sd(count), 
...
veg.nmds = ggplot(ds.veg, aes(x = NMDS1, y = NMDS2)) + 
    ...
    panel.background = element_blank(), panel.border = element_rect(colour = "black", fill = NA, size = 1.2),
    legend.key=element_blank()) + 
    labs(x = "NMDS1", colour = "Disturbance", shape = "Estuary", y = "NMDS2", caption = "Above-ground species presence/absence (Bray's distance), stress = 0.159")  + 
...
veg.nmds.ere = ggplot(ds.veg.ere, aes(x = NMDS1, y = NMDS2)) + 
    ...
    panel.background = element_blank(), panel.border = element_rect(colour = "black", fill = NA, size = 1.2),
    legend.key=element_blank()) + 
    labs(x = "NMDS1", colour = "Disturbance", y = "NMDS2", title = "Englishman", caption = "stress = 0.063")  + 
    theme(strip.text.x = element_text(size = 20, face = "bold"), plot.caption = element_text(size = 20)) +
...
veg.nmds.nre = ggplot(ds.veg.nre, aes(x = NMDS1, y = NMDS2)) + 
    ...
    panel.background = element_blank(), panel.border = element_rect(colour = "black", fill = NA, size = 1.2),
    legend.key=element_blank()) + 
    labs(x = "NMDS1", colour = "Disturbance", y = "NMDS2", title = "Nanaimo", caption = "stress = 0.087")  + 
    theme(strip.text.x = element_text(size = 20, face = "bold"), plot.caption = element_text(size = 20)) +
    scale_color_manual(limits = c("grubbed", "excl1", "excl10", "reference"), labels = c("Grubbed", "Heavily Grazed, \nExclosed 1 year", "Heavily Grazed, \nExclosed 10 years", "Undisturbed"), values = c("#762A83", "#C2A5CF", "#A6DBA0", "#1B7837"))
veg.nmds.nre
...
veg.nmds.lqre = ggplot(ds.veg.lqre, aes(x = NMDS1, y = NMDS2)) + 
    ...
    panel.background = element_blank(), panel.border = element_rect(colour = "black", fill = NA, size = 1.2),
    legend.key=element_blank()) + 
    labs(x = "NMDS1", colour = "Disturbance", y = "NMDS2", title = "Little Qualicum", caption = "stress = 0.039")  + 
    theme(strip.text.x = element_text(size = 20, face = "bold"), plot.caption = element_text(size = 20)) +
     scale_color_manual(limits = c("grubbed", "excl1", "excl10", "reference"), labels = c("Grubbed", "Heavily Grazed, \nExclosed 1 year", "Heavily Grazed, \nExclosed 10 years", "Undisturbed"), values = c("#762A83", "#C2A5CF", "#A6DBA0", "#1B7837"))
veg.nmds.lqre
...
```{r nmds seed bank comparison, facet wrap by estuary}

#################################
##1. join mie and seeds. mutate new column for seed vs. veg, change cover/count to presence/absence. 
#################################

# transform data into presence absence (pa)
seed_pa <- seed %>% 
  select(-c(collect, germ_obs, replicate, life, root, clade, fngp, abund.m2)) %>% 
...
pa_join <- mie_pa %>% 
  full_join(seed_pa, by = c("estuary", "endemic", "fngp2", "disturbance", "id", "obs", "agsp", "caly", "coco", "dece", "glma", "isce", "juba", "sade", "spca", "sysu")) %>% 
  replace(is.na(.), 0)
...
#################################
##2. PERMANOVA using species presence with Bray's distance 
#################################
# Does composition presence/absence significantly differ between seed bank and vegetation? 

# PERMANOVA
comp = pa_join[,5:ncol(pa_join)]
comp_matrix <- vegdist(comp, method = 'bray')

set.seed(42)
permanova <- adonis2(comp_matrix ~ disturbance + obs + estuary, data = pa_join, permutations = 99, method = "bray")
...
#add columns to data frame 
data.scores$id = pa_join$id
data.scores$disturbance = pa_join$disturbance
data.scores$obs = pa_join$obs
data.scores$estuary = pa_join$estuary
...
pa_nmds = ggplot(data.scores, aes(x = NMDS1, y = NMDS2)) + 
    ...
    panel.background = element_blank(), panel.border = element_rect(colour = "black", fill = NA, size = 1.2),
    legend.key=element_blank()) + 
    #geom_path(data=df_ell, aes(x=x, y=y, color = disturbance), size =2, alpha = 0.7) +
    labs(x = "NMDS1", colour = "Disturbance Condition", y = "NMDS2", shape = "Life Stage")  + 
    facet_grid(. ~ estuary, labeller = labeller(estuary = facet_labels)) + 
    theme(strip.text.x = element_text(size = 14, face = "bold")) +
    scale_colour_brewer(palette = "PRGn", limits = c("grubbed", "excl1", "excl10", "reference"), labels = c("Grubbed", "Heavily Grazed, \nExclosed 1 year", "Heavily Grazed, \nExclosed 10 years", "Undisturbed")) +
    scale_shape_discrete(limits = c("veg", "seed"), labels = c("Vegetation", "Summer Seed Bank"))
...
pa.join.ere <- pa_join %>% filter(estuary == "ere")
pa.join.nre <- pa_join %>% filter(estuary == "nre")
pa.join.lqre <- pa_join %>% filter(estuary == "lqre")
...
```{r relabund seed vs. veg}


#################################
##1. join mie and seeds. mutate new column for seed relative abundance 
#################################

# transform seed data into relative abundance (ra)
seed.select <- seed %>% 
  select(-c(collect, germ_obs, replicate, life, root, clade, abund.m2)) %>% 
...
seed.rel <- seed.select %>% 
  ...
  dplyr::summarize(n = n(),
                   sum.seed = sum(count),
                   seed.ra = (count/sum.seed)*100) 
#View(seed.rel)

seed.ra <- cbind(seed.select, "rel.abund" = seed.rel$seed.ra) 
#View(seed.ra)



# transform veg data into relative abundance (ra)
mie.select <- mie %>%
  select(-c(site, sal, elev, life, root, clade)) %>% 
  #filter(!species == "bare") %>% 
  group_by(id, endemic, fngp2)  %>% 
  mutate(obs = "veg") %>% 
  unite(endemic.fngp2, c(endemic, fngp2), sep = " ", remove = FALSE) 
...
mie.select$estuary <- as.character(mie.select$estuary)

mie.rel <- mie.select %>% 
  group_by(id) %>% 
  dplyr::summarize(n = n(),
                   sum.veg = sum(cover),
                   veg.ra = (cover/sum.veg)*100)

mie.ra <- cbind(mie.select, "rel.abund" = mie.rel$veg.ra) %>% 
...
ra.join <- mie.ra %>% 
  full_join(seed.ra, by = c("estuary", "disturbance", "id","species","obs","endemic.fngp2", "endemic","fngp", "fngp2","rel.abund")) %>% 
  select(-c("cover", "count", "endemic","fngp","fngp2")) %>% 
  unite(endemic.fngp2, c(endemic, fngp2), sep = " ", remove = FALSE)
#%>% 
  # pivot_wider(names_from = species, values_from = rel.abund) %>%
  # replace(is.na(.), 0) %>% 
  # filter_at(vars("atpa":"achmi"), any_vars(. != 0))
  # 
#View(ra.join)
...
pd.abund <- position_dodge(0.1)

abund.mean$estuary = factor(abund.mean$estuary, levels = c("ere", "nre", "lqre"))
abund.mean$disturbance = factor(abund.mean$disturbance,
                                          levels = c("grubbed", "excl1", "excl10", "reference"),
                                          labels = c("Grubbed", "Heavily Grazed, \nExclosed 1 year", "Heavily Grazed, \nExclosed 10 years", "Undisturbed"))

abund.seedveg <- abund.mean %>%  
  ggplot(aes(x=disturbance, y=sp.abund.mean, color = endemic, shape = obs)) +   
  ...
  #               width = .3, position = pd.abund) + 
  labs(x = "Disturbance condition", y = "Relative Abundance", color = "Species \nEndemic Status", shape = "Life Stage") +
  theme_classic()+
  theme(text = element_text(size = 20), 
          panel.spacing.x = unit(1.25, "lines"), 
          axis.text.x = element_text(angle = 25, hjust = 1)) +
  facet_grid(. ~ estuary, scales = "free", labeller = labeller(estuary = facet_labels)) +
  scale_color_manual(limits = c("Native", "Invasive", "NA"), labels = c("Native", "Invasive", "Unvegetated"), values = c("#01665E", "#BF812D", "#B3B3B3")) +
  scale_shape_manual(values = c(15, 17), limits = c("veg", "seed"), labels = c("Vegetation", "Seed Bank"))
abund.seedveg
...
# annual orange (native) "#FDBF6F" (invasive) "#FF7F00"
# forb purple (native) "#CAB2D6" (invasive) "#6A3D9A" 
#gramoind green (native) "#B2DF8A", (invasive) "#33A02C"
# Unvegetated = "#B3B3B3"

```

```{r relative abundances seed/veg, iteratively filter by endemic guild & life stage}
###############################################
### By PFG
###############################################

pfg.abund.mean <- ra.join %>% 
  group_by(estuary, disturbance, obs, endemic, endemic.fngp2) %>% 
  dplyr::summarize(n.abund = n(),
            sp.abund.mean = mean(rel.abund), 
            sp.abund.sd = sd(rel.abund), 
            sp.abund.se = sp.abund.sd/sqrt(n.abund)) %>% 
...
pfg.abund.mean$estuary = factor(pfg.abund.mean$estuary, levels = c("ere", "nre", "lqre"))
pfg.abund.mean$disturbance = factor(pfg.abund.mean$disturbance,
                                          levels = c("grubbed", "excl1", "excl10", "reference"),
                                          labels = c("Grubbed", "Heavily Grazed, \nExclosed 1 year", "Heavily Grazed, \nExclosed 10 years", "Undisturbed"))
pfg.abund.mean$endemic.fngp2 = factor(pfg.abund.mean$endemic.fngp2,
...
abund.col <- c("#FDBF6F", "#FF7F00", "#CAB2D6", "#6A3D9A", "#B2DF8A", "#33A02C") 

###############################################
# Veg, annuals
###############################################

abund.annual.veg <- pfg.abund.mean %>% 
  filter(obs == "veg" & endemic.fngp2 %in% c("Annual, Native", "Annual, Invasive"))

plot.abund.annual.veg <- abund.annual.veg %>%  
  ...
  scale_shape_manual(values = c(15, 17), limits = c("veg", "seed"), labels = c("Vegetation", "Seed Bank"))
plot.abund.annual.veg

ggsave("fig-abund-veg-ann.jpg", plot.abund.annual.veg, width = 15, height = 6)


###############################################
# Veg, annuals + perennial forbs
###############################################

abund.ann.per.veg <- pfg.abund.mean %>% 
  filter(obs == "veg" & endemic.fngp2 %in% c("Annual, Native", "Annual, Invasive", "Perennial Forb, Native", "Perennial Forb, Invasive"))

plot.abund.ann.per.veg <- abund.ann.per.veg %>%  
  ggplot(aes(x=disturbance, y=sp.abund.mean, color = endemic.fngp2, shape = obs)) +   
  geom_jitter(size = 7, position = pd.abund) +
  ylim(0, 100)+
  geom_errorbar(aes(ymin = sp.abund.mean - sp.abund.se, 
  ...
  scale_color_manual(values = abund.col) +
  scale_shape_manual(values = c(15, 17), limits = c("veg", "seed"), labels = c("Vegetation", "Seed Bank"))
plot.abund.ann.per.veg

ggsave("fig-abund-veg-ann-per.jpg", plot.abund.ann.per.veg, width = 15, height = 6)


###############################################
### All veg, by PFG
###############################################
pfg.abund.veg <- pfg.abund.mean %>% 
  filter(obs == "veg") 

#### plot 
plot.abund.veg <- pfg.abund.veg %>%  
  ggplot(aes(x=disturbance, y=sp.abund.mean, color = endemic.fngp2, shape = obs)) +   
  geom_jitter(size = 7, position = pd.abund) +
  ylim(0, 100)+
  geom_errorbar(aes(ymin = sp.abund.mean - sp.abund.se, 
...
ggsave("fig-abund-veg-all.jpg", plot.abund.veg, width = 15, height = 6)


###############################################
### All veg, and annual seeds 
###############################################


abund.allveg.ann.seed <- pfg.abund.mean %>% 
  filter(obs == "veg" | obs == "seed" & endemic.fngp2 %in% c("Annual, Native", "Annual, Invasive"))

#### plot 
plot.abund.veg.ann.seed <- abund.allveg.ann.seed %>%  
  ggplot(aes(x=disturbance, y=sp.abund.mean, color = endemic.fngp2, shape = obs)) +   
  geom_jitter(size = 7, position = pd.abund) +
  ylim(0, 100)+
  geom_errorbar(aes(ymin = sp.abund.mean - sp.abund.se, 
  ...
  scale_color_manual(values = abund.col) +
  scale_shape_manual(values = c(15, 17), limits = c("veg", "seed"), labels = c("Vegetation", "Seed Bank"))
plot.abund.veg.ann.seed

ggsave("fig-abund-veg-ann-seed.jpg", plot.abund.veg.ann.seed, width = 15, height = 6)

###############################################
### All veg, and annual + perennial forb seeds 
###############################################


abund.allveg.annper.seed <- pfg.abund.mean %>% 
  filter(obs == "veg" | obs == "seed" & endemic.fngp2 %in% c("Annual, Native", "Annual, Invasive", "Perennial Forb, Native", "Perennial Forb, Invasive"))

#### plot 
plot.abund.veg.annper.seed <- abund.allveg.annper.seed %>%  
  ggplot(aes(x=disturbance, y=sp.abund.mean, color = endemic.fngp2, shape = obs)) +   
  geom_jitter(size = 7, position = pd.abund) +
  ylim(0, 100)+
  geom_errorbar(aes(ymin = sp.abund.mean - sp.abund.se, 
  ...
  scale_color_manual(values = abund.col) +
  scale_shape_manual(values = c(15, 17), limits = c("veg", "seed"), labels = c("Vegetation", "Seed Bank"))
plot.abund.veg.annper.seed

ggsave("fig-abund-veg-ann-per-seed.jpg", plot.abund.veg.annper.seed, width = 15, height = 6)
...
#### plot 
pfg.abund.seedveg <- pfg.abund.mean %>%  
  ggplot(aes(x=disturbance, y=sp.abund.mean, color = endemic.fngp2, shape = obs)) +   
  geom_jitter(size = 7, position = pd.abund) +
  ylim(0, 100)+
  geom_errorbar(aes(ymin = sp.abund.mean - sp.abund.se, 
...
ggsave("fig-abund-seedveg.jpg", pfg.abund.seedveg, width = 15, height = 6)


##############################################################
## Extra code, leftover (del if not needed when running from empty environment)
##############################################################
...
```{r NMDS relabund seed vs. veg}

#################################
## NMDS of species relative abundance using ra.join 
#################################
join.ra.ere <- ra.join %>% filter(estuary == "ere") %>% 
  pivot_wider(names_from = species, values_from = rel.abund) %>%
  replace(is.na(.), 0) 

join.ra.nre <- ra.join %>% filter(estuary == "nre") %>% 
  pivot_wider(names_from = species, values_from = rel.abund)%>%
  replace(is.na(.), 0) 

join.ra.lqre <- ra.join %>% filter(estuary == "lqre") %>% 
  pivot_wider(names_from = species, values_from = rel.abund) %>%
  replace(is.na(.), 0) 
...
ra.com.ere = join.ra.ere[,8:ncol(join.ra.ere)]
ra.com.nre = join.ra.nre[,8:ncol(join.ra.nre)]
ra.com.lqre = join.ra.lqre[,8:ncol(join.ra.lqre)]
#View(join.ra.nre)

#turn abundance data frame into a matrix
m.ra.ere = as.matrix(ra.com.ere)
m.ra.nre = as.matrix(ra.com.nre)
m.ra.lqre = as.matrix(ra.com.lqre)

set.seed(123)
nmds.seedvegra.ere = metaMDS(m.ra.ere, distance = "bray")
nmds.seedvegra.ere
# stress = 0.0002

set.seed(123)
nmds.seedvegra.nre = metaMDS(m.ra.nre, distance = "bray")
nmds.seedvegra.nre
# stress = 0.0003

set.seed(123)
nmds.seedvegra.lqre = metaMDS(m.ra.lqre, distance = "bray")
nmds.seedvegra.lqre
# stress = 0.0006


#extract NMDS data scores (x and y coordinates)
ds.seedvegra.ere = as.data.frame(scores(nmds.seedvegra.ere))
ds.seedvegra.nre = as.data.frame(scores(nmds.seedvegra.nre))
ds.seedvegra.lqre = as.data.frame(scores(nmds.seedvegra.lqre))


#add columns to data frame 
ds.seedvegra.ere$id = join.ra.ere$id
ds.seedvegra.nre$id = join.ra.nre$id
ds.seedvegra.lqre$id = join.ra.lqre$id


ds.seedvegra.ere$disturbance = join.ra.ere$disturbance
ds.seedvegra.nre$disturbance = join.ra.nre$disturbance
ds.seedvegra.lqre$disturbance = join.ra.lqre$disturbance


ds.seedvegra.ere$obs = join.ra.ere$obs
ds.seedvegra.nre$obs = join.ra.nre$obs
ds.seedvegra.lqre$obs = join.ra.lqre$obs


ds.seedvegra.ere$estuary = join.ra.ere$estuary
ds.seedvegra.nre$estuary = join.ra.nre$estuary
ds.seedvegra.lqre$estuary = join.ra.lqre$estuary

```

```{r plot NMDS seed veg ra, ggarrange by estuary}
...
## ERE
ds.seedvegra.ere$disturbance <- as.factor(ds.seedvegra.ere$disturbance)

seedvegra.nmds.ere <- ds.seedvegra.ere %>% 
    ggplot(aes(x = NMDS1, y = NMDS2, shape = obs, colour = disturbance)) + 
    geom_point(size = 6)+ 
    #xlim(-1,1.5) + ylim(-1,1) +
    theme(axis.text.y = element_text(colour = "black", size = 20, face = "bold"), 
    axis.text.x = element_text(colour = "black", face = "bold", size = 20), 
    ...
    panel.background = element_blank(), panel.border = element_rect(colour = "black", fill = NA, size = 1.2),
    legend.key=element_blank()) + 
    labs(x = "NMDS1", colour = "Disturbance", shape = "Life Stage", y = "NMDS2", title = "Englishman", caption = "stress = 0.0002")  + 
    theme(strip.text.x = element_text(size = 20, face = "bold"), plot.caption = element_text(size = 20)) +
    scale_color_manual(limits = c("grubbed", "excl1", "excl10", "reference"), labels = c("Grubbed", "Heavily Grazed, \nExclosed 1 year", "Heavily Grazed, \nExclosed 10 years", "Undisturbed"), values = c("#762A83", "#C2A5CF", "#A6DBA0", "#1B7837")) +
  scale_shape_manual(values = c(15, 17), limits = c("veg", "seed"), labels = c("Vegetation", "Seed Bank"))
seedvegra.nmds.ere
...
## NRE
ds.seedvegra.nre$disturbance <- as.factor(ds.seedvegra.nre$disturbance)

seedvegra.nmds.nre = ggplot(ds.seedvegra.nre, aes(x = NMDS1, y = NMDS2)) + 
    geom_point(size = 6, aes(shape = obs, colour = disturbance))+ 
    #xlim(-1,1.7) + ylim(-1,1) +
    theme(axis.text.y = element_text(colour = "black", size = 20, face = "bold"), 
    axis.text.x = element_text(colour = "black", face = "bold", size = 20), 
    ...
    panel.background = element_blank(), panel.border = element_rect(colour = "black", fill = NA, size = 1.2),
    legend.key=element_blank()) + 
    labs(x = "NMDS1", colour = "Disturbance", shape = "Life Stage", y = "NMDS2", title = "Nanaimo", caption = "stress = 0.0003")  + 
    theme(strip.text.x = element_text(size = 20, face = "bold"), plot.caption = element_text(size = 20)) +
    scale_color_manual(limits = c("grubbed", "excl1", "excl10", "reference"), labels = c("Grubbed", "Heavily Grazed, \nExclosed 1 year", "Heavily Grazed, \nExclosed 10 years", "Undisturbed"), values = c("#762A83", "#C2A5CF", "#A6DBA0", "#1B7837")) +
  scale_shape_manual(values = c(15, 17), limits = c("veg", "seed"), labels = c("Vegetation", "Seed Bank"))
seedvegra.nmds.nre
...
## LQRE
ds.seedvegra.lqre$disturbance <- as.factor(ds.seedvegra.lqre$disturbance)

seedvegra.nmds.lqre = ggplot(ds.seedvegra.lqre, aes(x = NMDS1, y = NMDS2)) + 
    geom_point(size = 6, aes(shape = obs, colour = disturbance))+ 
    #xlim(-1,1.5) + ylim(-1,1) +
    theme(axis.text.y = element_text(colour = "black", size = 20, face = "bold"), 
    axis.text.x = element_text(colour = "black", face = "bold", size = 20), 
    ...
    panel.background = element_blank(), panel.border = element_rect(colour = "black", fill = NA, size = 1.2),
    legend.key=element_blank()) + 
    labs(x = "NMDS1", colour = "Disturbance", shape = "Life Stage", y = "NMDS2", title = "Little Qualicum", caption = "stress = 0.0006")  + 
    theme(strip.text.x = element_text(size = 20, face = "bold"), plot.caption = element_text(size = 20)) +
    scale_color_manual(limits = c("grubbed", "excl1", "excl10", "reference"), labels = c("Grubbed", "Heavily Grazed, \nExclosed 1 year", "Heavily Grazed, \nExclosed 10 years", "Undisturbed"), values = c("#762A83", "#C2A5CF", "#A6DBA0", "#1B7837")) +
  scale_shape_manual(values = c(15, 17), limits = c("veg", "seed"), labels = c("Vegetation", "Seed Bank"))
seedvegra.nmds.lqre
...
## panel format w/ ggpubr
seedvegra.nmds.panel <- ggarrange(seedvegra.nmds.ere, seedvegra.nmds.nre, seedvegra.nmds.lqre,
          common.legend = TRUE, legend = "bottom",
          ncol = 3, nrow = 1) %>% 
  annotate_figure(bottom = text_grob("Compositional similarity of seed bank vs. above-ground vegetation based on functional group relative abundance (Bray's distance)", hjust = 1, x = 1,  size = 14))
seedvegra.nmds.panel

#ggsave("fig-nmds-seedvegra-panel.jpg", seedvegra.nmds.panel, width = 20, height = 6)
