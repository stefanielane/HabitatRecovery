...
seed <- seed_ra %>% 
  ...
  mutate(root = ifelse(species %in% c("atpa", "cirsp", "coco", "dece", "grsp", "jute", "isce", "sade", "sama","poasp"), "Fibrous", "Rhizomatous")) %>%
  ...
  mutate(structure = ifelse(species %in% c("coco", "isce" , "glma", "elpar", "popa", "sade", "sama", "spca"), "short", 
                  ifelse (species %in% c("achmi", "agsp", "atpa", "epgl", "grsp", "juar", "juba", "juen", "jute", "sysu", "trma"), "med", "tall")))  # short = < 50 cm; med = 50 - 1 m, tall = > 1 m, per eflora BC
...
stacked.bar.panel <- annotate_figure(stacked.bar.panel, 
