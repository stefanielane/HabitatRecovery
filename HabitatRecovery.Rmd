...
# 7 x 266
#View(seed_ra)
unique(seed_ra$species)

seed <- seed_ra %>% 
  mutate(obs = "seed", id = (seed_id+100)) %>% 
  mutate(endemic = ifelse(species %in% c("agsp", "atpa", "coco", "cirsp", "juge", "poasp"), "Invasive", "Native")) %>% 
  mutate(life = ifelse(species %in% c("atpa", "isce","jubu", "sade", "spca", "poasp"), "Annual", "Perennial")) %>% 
  mutate(root = ifelse(species %in% c("atpa", "cirsp", "coco", "dece", "grsp", "isce", "sade", "sama","poasp"), "Fibrous", "Rhizomatous")) %>%
  mutate(clade = ifelse(species %in% c("agsp", "caly", "dece", "juar", "juba", "juen", "juge", "poasp"), "Select Graminoid",
                        ifelse(species %in% c("agsp", "caly", "dece", "elpar", "isce", "juar", "juba", "juen", "juge", "poasp"), "All Graminoid", "Forb"))) %>% 
  mutate(fngp = ifelse(species %in% c("agsp", "caly", "elpar", "juar", "juba", "juen", "juge"), "Perennial graminoid, rhizomatous roots",                  
                       ifelse(species %in% c("dece"), "Perennial graminoid,  fibrous roots",
                       ifelse(species %in% c("atpa", "sade", "spca"), "Annual forb", "Perennial forb")))) %>% 
   mutate(fngp2 = ifelse(species %in% c("agsp", "caly", "juar", "juen", "juba", "juge", "dece"), "Perennial graminoid (> 10 cm)",
                         ifelse(species %in% c("elpar"), "Perennial graminoid (< 10 cm)",
  ...
  mutate(structure = ifelse(species %in% c("coco", "isce" , "glma", "elpar", "juge", "popa", "sade", "sama", "spca"), "short", 
...
```{r summarize vegetation (veg.sum, seed.sum, all.sum)}
...
ag.sum <- veg.sum %>% 
                  select(c(id, disturbance, estuary, graminoid_prop, obs))
...
```{r TABLE 2. indicator species analysis by disturbance}

library(indicspecies)
# see https://cran.r-project.org/web/packages/indicspecies/vignettes/IndicatorSpeciesAnalysis.html#defining-the-classification-of-sites
...
indsp.ag$disturbance <- factor(indsp.ag$disturbance, levels = c("reference", "excl10", "excl1", "grubbed"))
          
abund.ag = indsp.ag[,5:ncol(indsp.ag)]
assoc.ag = indsp.ag$disturbance

inv.ag_r.g = multipatt(abund.ag, assoc.ag, 
                       func = "r.g", # can you explain why you used this func?
                       max.order = 1, #restricts groups. use max.order = 1 to only see indicsp for each disturbance type
                       control = how(nperm=999))
summary(inv.ag_r.g)
# summary(inv.ag_r.g, alpha=1)
# inv.ag_r.g$sign
...
indsp.bg$disturbance <- factor(indsp.bg$disturbance, levels = c("reference", "excl10", "excl1", "grubbed"))

          
abund.bg = indsp.bg[,5:ncol(indsp.bg)]
assoc.bg = indsp.bg$disturbance

inv.bg_r.g = multipatt(abund.bg, assoc.bg, 
                       func = "r.g", 
                       max.order = 1,
                       control = how(nperm=999))
summary(inv.bg_r.g)
...
```{r FIG. 3. boxplots - veg richness & abundance: native vs exotic}
...
agveg <- veg.sum %>% 
        select(id, disturbance, estuary, native_rich, exotic_rich, native_cover, exotic_cover) %>% 
        dplyr::rename(native_abund = native_cover, 
               exotic_abund = exotic_cover) %>% 
        mutate(obs = "veg")

bgssb <- seed.sum %>% 
        select(id, disturbance, estuary, native_rich, exotic_rich, native_relabund, exotic_relabund) %>% 
        dplyr::rename(native_abund = native_relabund, 
                      exotic_abund = exotic_relabund) %>% 
        mutate(obs = "seed")
...
agbg_box <- agbg %>%
  pivot_longer(native_rich:exotic_rich,
               names_to = "richness",
               values_to = "rich_val") %>%
  pivot_longer(native_abund:exotic_abund,
               names_to = "abund",
               values_to = "abund_val")


agbg_box$estuary = factor(agbg_box$estuary, 
                         levels = c("nre", "lqre"), 
                         labels = c("Nanaimo", "Little Qualicum"))

agbg_box$obs = factor(agbg_box$obs, 
                         levels = c("veg", "seed"), 
                         labels = c("Above-ground Vegetation", "Surface Seed Bank"))

agbg_box$disturbance = factor(agbg_box$disturbance,
...
agbg_box$richness = factor(agbg_box$richness,
                            levels = c("native_rich", "exotic_rich"),
                            labels = c("Native", "Non-native"))

agbg_box$abund = factor(agbg_box$abund,
                            levels = c("native_abund", "exotic_abund"),
                            labels = c("Native", "Non-native"))

pd <- position_dodge(0.75)

rich_box <- agbg_box %>%
  ggplot(aes(x = estuary, y = rich_val, fill = richness)) +
  geom_boxplot(outlier.shape = NA) + 
  geom_point(position = position_jitterdodge(), 
              alpha = 0.5) +
  theme_classic() +
  facet_grid(obs ~ disturbance, scales = "free_x") + 
  ...
  labs(x = "", y = "Total Species Richness", fill = "") +
...
#ggsave("fig-box-agbg-richness.jpg", rich_box, width = 10, height = 8)

#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~
# richness boxplots with shaded panel background
#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~

# veg_box <- agbg_box %>%
#   filter(obs == "Above-ground Vegetation") %>% 
#   ggplot(aes(x = disturbance, y = rich_val, fill = richness)) +
#   geom_boxplot(outlier.shape = NA) + 
#   geom_point(position = position_jitterdodge(), 
#               alpha = 0.5) +
#   # geom_rect(aes(fill = obs),
...
#   labs(x = "Disturbance condition", y = "Total Species Richness", fill = "Endemism") +
#   scale_y_continuous(limits = c(0, 8), breaks = seq(0, 8, 2)) +
#   theme(text = element_text(size = 18),
#         panel.spacing.x = unit(1.25, "lines"),
#         axis.text.x = element_text(angle = 25, hjust = 1)) +
...
#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~
# native/non-native abundance boxplots (all species)
#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~

abund_box <- agbg_box %>%
  ggplot(aes(x = disturbance, y = abund_val, fill = abund)) +
  geom_boxplot() +
  theme_classic() +
  facet_grid(obs ~ estuary, scales = "free") +
...
#ggsave("fig-box-agbg-cover.jpg", cover_box, width = 8, height = 6)



```

```{r FIG. 4. boxplots of native vs exotic tall, perennial graminoids in veg vs seed}
...
```{r FIG. 5 - grouped barchart AG/BG relabund}
...
indic_sp_ag <- ra_join %>% 
              select(estuary, disturbance, species, relabund, obs, id) %>% 
              filter(obs == "veg",
                     species %in% c("agsp",
                                    "caly",
                                    "coco",
              ...
              replace(is.na(.), 0) %>% 
              mutate(juar = 0) %>% 
              pivot_longer(agsp:juar, names_to = "species", values_to = "relabund")
...
#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~
## Plot Above-ground Vegetation
#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~

indic_sp_ag$disturbance = factor(indic_sp_ag$disturbance,
                            levels = c("grubbed", "excl1", "excl10", "reference"),
                            labels = c("Grubbed", "Heavily Grazed, \nExclosed 1 year", "Heavily Grazed, \nExclosed 10 years", "Undisturbed")) 

indic_sp_ag$estuary = factor(indic_sp_ag$estuary,
                            levels = c("nre", "lqre"),
                            labels = c("Nanaimo", "Little Qualicum")) 

indic_sp_ag$obs <- factor(indic_sp_ag$obs, 
                          levels = c("veg"),
                          labels = c("Above-ground vegetation"))
ag <- ggplot(indic_sp_ag, 
  ...
  scale_fill_manual(values = c("goldenrod1","lightgoldenrod1", "#7FCDBB" , "#1D91C0" )) + #"#FFD92F",  "#7FCDBB" , "#1D91C0" 
  scale_x_discrete(labels=c("agsp" = "Agrostis \nstolonifera*", 
                            "caly" = "Carex \nlyngbyei",
                            "coco" = "Cotula \ncoronopifolia*",
                            "elpar" = "Eleocharis \nparvula",
                            "glma" = "Glaux \nmaritima",
                            ...
                            "sysu" = "Symphyotrichum \nsubspicatum",
                            "trma" = "Triglochin \nmaritima")) +
  facet_grid(obs~estuary) +
  theme(panel.spacing.x = unit(1, "cm"))

#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~
## Plot Surface Seed Bank
#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~

indic_sp_bg$disturbance = factor(indic_sp_bg$disturbance,
                            levels = c("grubbed", "excl1", "excl10", "reference"),
                            labels = c("Grubbed", "Heavily Grazed, \nExclosed 1 year", "Heavily Grazed, \nExclosed 10 years", "Undisturbed")) 

indic_sp_bg$estuary = factor(indic_sp_bg$estuary,
                            levels = c("nre", "lqre"),
                            labels = c("Nanaimo", "Little Qualicum")) 

indic_sp_bg$obs <- factor(indic_sp_bg$obs, 
                          levels = c("seed"),
                          labels = c("Surface seed bank"))
...
bg <- ggplot(indic_sp_bg, 
    ...
    scale_fill_manual(values = c("goldenrod1","lightgoldenrod1", "#7FCDBB" , "#1D91C0" )) +
...
stacked.bar.panel <- ggarrange(ag, bg,
                          #labels = c("Above-ground vegetation", "Surface seed bank"),
...
annotate_figure(stacked.bar.panel, 
                left = text_grob("Relative abundance (%)", 
                                 size = 25, 
                                 rot = 90))

ggsave("fig-stacked-relabund.jpg", stacked.bar.panel, width = 25, height = 15)
...
veg.sum$disturbance <- factor(veg.sum$disturbance, 
                              levels = c("reference", "grubbed", "excl1", "excl10"))
veg.sum$estuary <- factor(veg.sum$estuary)

# (*can add estuary as a fixed effect (+ estuary); show no influence as supplement
ag_glm <- glm(graminoid_prop ~ disturbance, 
                 family = 'binomial',
                 data = veg.sum) # binomial family appropriate for proportional data constrained 0:1. logit link is default.  

summary(ag_glm) # transform coefficients? see fn tab_model

summary(ag_glm)$coefficients

plot(ag_glm)


# per Erin; see https://strengejacke.github.io/ggeffects/reference/ggpredict.html
#ag_tpg_plot <- ggpredict(ag_glm, terms = c("disturbance"), type = "fe")

# make this into your mean & SE, see how it compares to ggpredict -> can you explain what predict or ggpredict is doing?
ag_pred_mean <- predict(ag_glm, type = "response") 
veg.sum$gram_prop_mean = ag_pred_mean
#View(veg.sum)


ag_pred_se <- predict(ag_glm, type = c("response"), se.fit = TRUE)$se.fit
veg.sum$se = ag_pred_se


veg_mod <- veg.sum %>%
                mutate(gram_prop_se_max = gram_prop_mean+se,
                       gram_prop_se_min = gram_prop_mean-se) %>% 
                select(disturbance, estuary, graminoid_prop, gram_prop_mean, gram_prop_se_max, gram_prop_se_min) %>% 
                mutate(obs = "Above-ground vegetation")

#View(veg_mod)
veg_mod$disturbance <- factor(veg_mod$disturbance, 
                      level = c("grubbed", "excl1", "excl10", "reference"),
                      labels = c(grubbed = "Grubbed", excl1 = "Heavily Grazed, \nExclosed 1 year", excl10 = "Heavily Grazed, \nExclosed 10 years", reference = "Undisturbed"))

veg_mod$estuary = factor(veg_mod$estuary, 
                         levels = c("nre", "lqre"), 
                         labels = c("Nanaimo", "Little Qualicum"))

ag_model <- ggplot() +
  geom_jitter(data = veg_mod,
              aes(x = estuary,
                  y = graminoid_prop, 
                  color = obs),
                  size = 4) +
  scale_color_manual(values = "#1D91C0") +
  geom_errorbar(data = veg_mod,
                aes(x = estuary, 
                    ymin = ifelse(gram_prop_se_min < 0, 0, gram_prop_se_min), 
                    ymax = ifelse(gram_prop_se_max > 100, 100, gram_prop_se_max)),
                linewidth = 2,
                width = 0.5) + #position=position_dodge(width=0.5)
   geom_point(data = veg_mod, 
             aes(x = estuary, 
                 y = gram_prop_mean), 
             size = 4,
             position=position_dodge(width=0.5)) +
  theme_classic() +
  facet_grid(obs~disturbance, 
             #nrow = 1,
             scales = "free_x") +
...
# make this into your mean & SE, see how it compares to ggpredict -> can you explain what ggpredict is doing?
bg_pred_mean <- predict(seed_glm, type = "response") 
seed.sum$gram_prop_mean = bg_pred_mean
#View(seed.sum)


bg_pred_se <- predict(seed_glm, type = c("response"), se.fit = TRUE)$se.fit
seed.sum$se = bg_pred_se


seed_mod <- seed.sum %>%
                mutate(gram_prop_se_max = gram_prop_mean+se,
                       gram_prop_se_min = gram_prop_mean-se) %>% 
                mutate(obs = "Surface seed bank")

seed_mod$disturbance <- factor(seed_mod$disturbance, 
                      level = c("grubbed", "excl1", "excl10", "reference"),
                      labels = c(grubbed = "Grubbed", excl1 = "Heavily Grazed, \nExclosed 1 year", excl10 = "Heavily Grazed, \nExclosed 10 years", reference = "Undisturbed"))

seed_mod$estuary = factor(seed_mod$estuary, 
                         levels = c("nre", "lqre"), 
                         labels = c("Nanaimo", "Little Qualicum"))
...
bg_model <- ggplot() +
  geom_jitter(data = seed_mod, 
              aes(x = estuary, 
                  y = graminoid_prop, 
                  color = obs),
              size = 4) + 
    scale_color_manual(values = "#1D91C0") +
    geom_errorbar(data = seed_mod,
                aes(x = estuary, 
                    ymin = ifelse(gram_prop_se_min < 0, 0, gram_prop_se_min), 
                    ymax = gram_prop_se_max,
                width = 0.5),
                linewidth = 2) + # position=position_dodge(width=0.5)
     geom_point(data = seed_mod, 
             aes(x = estuary, 
                 y = gram_prop_mean), 
             size = 4,
             position=position_dodge(width=0.5)) +
  ...
  labs(x = "", 
        y = "") #+
  # scale_color_discrete(name = "Estuary", 
...
```{r FIGURE - actual vs. predicted values for AG/BG}

model.panel <- ggarrange(ag_model, NULL, bg_model,
                          #labels = c("Above-ground vegetation", "Surface seed bank"),
                          #font.label=list(color="black", size=16),
                          #common.legend = TRUE, 
                          #legend = "right",
                          #ncol = 1, 
                          nrow = 3, 
                          heights = c(1, 0.25, 1.25)) 


annotate_figure(model.panel, 
                left = text_grob("Proportion of tall, perennial graminoids", 
                                 size = 30, 
                                 rot = 90))
...
nomenclature <- read.csv("species.csv") %>% 
                select(species, code) %>% 
                dplyr::rename(Latin = species)

allsp_table <- ra_join %>% 
              filter(!species == "bare") %>% 
              left_join(nomenclature, by = c("species" = "code")) %>% 
              select(estuary, obs, species, relabund, Latin) %>% 
              group_by(estuary, obs, Latin) %>% 
              dplyr::summarize(mean_relabund = mean(relabund)) %>% 
              pivot_wider(names_from = obs, values_from = mean_relabund)
View(allsp_table)

library(writexl)
write_xlsx(allsp_table, "table_species-relabund.xlsx", col_names = TRUE)
...
```{r grouped bar charts - all species}

# need to figure out how to show all species on x axis, but still have seed banks extending down. 
...
all_bg <- ra_join %>% 
              ...
              pivot_wider(names_from = species, values_from = relabund) %>% 
              replace(is.na(.), 0) %>% 
              pivot_longer(agsp:juen, names_to = "species", values_to = "relabund") %>% 
              mutate(relabund = relabund * -1)
...
#ggsave("fig-bar-agbg-relabund.jpg", all_bar, width = 15, height = 8)


#~#~#~#~#~#~#~#~#~#~#~#~#~#~#

# ag_bar <- ggplot(all_ag, 
#          aes (x = species, 
#               y = relabund, 
#               fill = disturbance)) +
#   geom_bar(stat="identity",
...
#   scale_fill_brewer(palette = "PRGn") +
#   #facet_grid(cols = vars(estuary), scales = "free_x", space = "free_x")
#   facet_wrap(~estuary, scales = "free_x") +
...
#    scale_fill_brewer(palette = "PRGn") +
#   facet_wrap(~estuary, scales = "free_x") +
#   scale_y_continuous(breaks = seq(-100, 0, by=25), limits=c(-100, 0)) +
#   theme(panel.spacing = unit(1, "cm", data = NULL))


```

```{r SUPPLEMENTAL AG/BG diversity}

agDiv <- mie %>% 
          filter(!cover == 0 & !species == "bare") %>% 
          select(c("estuary", "disturbance", "id", "species", "obs"))

bgDiv <- seed %>% 
          select(c("estuary", "disturbance", "id", "species", "obs"))

div <- rbind(agDiv, bgDiv) %>% 
        group_by(estuary, disturbance, obs) %>% 
        dplyr::summarise(div = n_distinct(species))
#View(div)



div$disturbance = factor(div$disturbance,
                            levels = c("grubbed", "excl1", "excl10", "reference"),
                            labels = c("Grubbed", "Heavily Grazed, \nExclosed 1 year", "Heavily Grazed, \nExclosed 10 years", "Undisturbed")) 

div$estuary = factor(div$estuary,
                            levels = c("nre", "lqre"),
                            labels = c("Nanaimo", "Little Qualicum")) 

div$obs = factor(div$obs, 
                      levels = c("veg", "seed"),
                      labels = c("Above-ground vegetation", "Surface seed bank"))
...
div %>% 
  ...
  geom_hline(yintercept=0) +
  theme_classic() +
  labs(y = "total species diversity", 
       x = "", 
       fill = "Disturbance") +
  theme(axis.text.x = element_blank()) +
  scale_fill_brewer(palette = "PRGn") +
  facet_grid(obs~estuary, scales = "free_x") +
  theme(panel.spacing = unit(1, "cm", data = NULL))
...
```{r glm - AGBG TPG}
# tpg as proportion of relabund, use all.sum for "obs" as factor

###################################
## WARNING
## this chunk works, and afaik this glm is valid, however chapter/ms talks about models & results seprately. 
## Therefore, this chunk is redundant 
###################################


ggplot(all.sum, aes(x = graminoid_prop)) +
  geom_histogram(position = "dodge", binwidth = 0.1) + 
  theme_classic()


all.sum$disturbance <- factor(all.sum$disturbance, levels = c("reference", "grubbed", "excl1", "excl10"))
all.sum$estuary <- factor(all.sum$estuary)
all.sum$obs <- factor(all.sum$obs, levels = c("veg", "seed"))


all_glm <- glm(graminoid_prop ~ disturbance + estuary + obs, 
                 family = 'binomial',
                 data = all.sum) 

nonestuary_glm <- glm(graminoid_prop ~ disturbance + obs, 
                 family = 'binomial',
                 data = all.sum) 

summary(all_glm) 
# NRE has near-significantly fewer seeds of TPG than LQRE (this may be due to fewer exotic AGST found in seed bank at NRE); grubbed sites have fewer TPG seed than reference, although not stat sig. 

summary(all_glm)$coefficients
# Because numerical output of the model corresponds to log-odds, coefficient of intercept is the log-odds of observing TPG proportion in the vegetation at Undisturbed sites in LQRE. Confirm: is it fair to say the Grubbed sites have 5x lower proportion of TPGs (-5.3 intercept)?
## Note: intercepts are point estimates. Need to estimate a confidence level. 

plot(all_glm)

# summary(nonestuary_glm) 
# # 
# summary(nonestuary_glm)$coefficients
# plot(nonestuary_glm)


# make this into your mean & SE, see how it compares to ggpredict -> can you explain what ggpredict is doing?

# type = "response" returns the probability (not log-odds) of finding TPG proportions similar to undisturbed sites
agbg_pred_mean <- predict(all_glm, type = "response") 

all.sum$gram_prop_mean = agbg_pred_mean
#View(all.sum)


agbg_pred_se <- predict(all_glm, 
                        type = c("response"), 
                        se.fit = TRUE)$se.fit
all.sum$se = agbg_pred_se
...
level_order <- factor(all.sum$disturbance, 
                      level = c("grubbed", "excl1", "excl10", "reference"),
                      labels = c(grubbed = "Grubbed", excl1 = "Heavily Grazed, \nExclosed 1 year", excl10 = "Heavily Grazed, \nExclosed 10 years", reference = "Undisturbed"))

agbg_model <- ggplot() +
  geom_jitter(data = all.sum, 
              aes(x = level_order, 
                  y = graminoid_prop,
                  color = estuary, 
                  shape = obs),
              size = 4) + 
    geom_errorbar(data = all.sum,
                aes(x = level_order, 
                    ymin = ifelse(gram_prop_se_min < 0, 0, gram_prop_se_min), 
                    ymax = gram_prop_se_max, 
                    color = estuary),
  ...
  labs(x = "", 
       y = "") +
  scale_color_discrete(name = "Estuary", labels = c("Little Qualicum", "Nanaimo"))
...
```{r glm - AG & BG same model}
# use ra_join from Chunk 6

#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#
## WARNING: 
## This chunk appears to be a duplicate/experimental attempt. 
## Note that 'ra_join' does not have column graminoid_prop
#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#

# https://towardsdatascience.com/random-effects-in-linear-models-15845b2540ac
# basics of glms https://www.geo.fu-berlin.de/en/v/soga/Basics-of-statistics/Logistic-Regression/Logistic-Regression-in-R---An-Example/index.html 

ggplot(ra_join, aes(x = graminoid_prop)) +
  geom_histogram(position = "dodge", binwidth = 0.1) + 
  theme_classic() 
