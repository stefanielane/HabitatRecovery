...
pfg.abund.ere <- pfg2.ere %>% 
  dplyr::summarize(veg.fngp2.n = n(),
           ...
           veg.fngp2.sd = sd(relabund), 
            veg.fngp2.se = veg.fngp2.sd/sqrt(veg.fngp2.sd)) %>% 
...
pfg2.panel <- ggarrange(pfg2.ere, pfg2.nre, pfg2.lqre,
...
seed_pa <- seed %>% 
  select(-c(collect, germ_obs, replicate, life, root, clade, fngp, endemic, abund.m2)) %>% 
...
mie_pa <- mie %>%
  select(-c(site, sal, elev, endemic, life, root, clade, fngp, fngp2)) %>% 
...
pa_join <- mie_pa %>% 
  full_join(seed_pa, by = c("estuary", "disturbance", "id", "obs", "agsp", "caly", "coco", "dece", "glma", "isce", "juba", "sade", "spca", "sysu")) %>% 
  replace(is.na(.), 0)
...
# matrix of species presence/absence
pa.com.ere = pa.join.ere[,5:ncol(pa.join.ere)]
pa.com.nre = pa.join.nre[,5:ncol(pa.join.nre)]
pa.com.lqre = pa.join.lqre[,5:ncol(pa.join.lqre)]
...
# transform data into relative abundance (ra)
seed.select <- seed %>% 
...
seed.rel <- seed.select %>% 
  ...
  dplyr::summarize(n = n(),
                   sum.seed = sum(count),
                   seed.ra = count/sum.seed) 
View(seed.rel)

seed.ra <- cbind(seed.select, "rel.abund" = seed.rel$seed.ra) %>% 
  pivot_wider(names_from = endemic.fngp2, values_from = rel.abund) %>% 
  mutate_all(~replace(., is.na(.), 0))  %>% 
  select(-count)
View(seed.ra)  
colnames(seed.ra)
# "estuary", "disturbance", "id","species","obs","endemic","fngp","fngp2", "Native Annual", "Invasive Perennial forb", "Native Perennial forb" ,"Native Perennial graminoid" , "Invasive Perennial graminoid"
...
ra.join <- mie.ra %>% 
  full_join(seed.ra, by = c("estuary", "disturbance", "id","species","obs","endemic","fngp","fngp2", "Native Annual", "Invasive Perennial forb", "Native Perennial forb" ,"Native Perennial graminoid" , "Invasive Perennial graminoid")) %>% 
  replace(is.na(.), 0)


#filter all rows with species values == 0, then remove all columns with all values == 0
ra.join.ere <- ra.join %>% 
  filter(estuary == "ere") %>% 
  filter_at(vars("Native Annual":"Invasive Perennial graminoid"), any_vars(. != 0))
ra.ere <- ra.join.ere[, colSums(ra.join.ere != 0) > 0]

ra.join.nre <- ra.join %>% 
  filter(estuary == "nre") %>% 
  filter_at(vars("Native Annual":"Invasive Perennial graminoid"), any_vars(. != 0))
ra.nre <- ra.join.nre[, colSums(ra.join.nre != 0) > 0]

ra.join.lqre <- ra.join %>% 
  filter(estuary == "lqre") %>% 
  filter_at(vars("Native Annual":"Invasive Perennial graminoid"), any_vars(. != 0))
ra.lqre <- ra.join.lqre[, colSums(ra.join.lqre != 0) > 0]
#View(ra.lqre)
...
#summarize mean & SE
pfgra.abund <- ra.join %>% 
  pivot_longer(c("Native Annual", "Invasive Perennial forb", "Native Perennial forb" ,"Native Perennial graminoid" , "Invasive Perennial graminoid"), names_to = "endemic.fngp", values_to = "relabund") %>% 
  dplyr::summarize(endemic.fngp = n(),
            endemic.fngp.mean = mean(relabund), 
           endemic.fngp.sd = sd(relabund), 
            endemic.fngp.se = endemic.fngp.sd/sqrt(endemic.fngp.sd)) %>% 
    mutate_all(~replace(., is.na(.), 0)) 
# 
# pfgra.abund$estuary = ra.join.ere$estuary
# pfgra.abund$disturbance = ra.join.ere$disturbance

pfgra.ere <- pfgra.abund %>% filter(estuary == "ere")
pfgra.nre <- pfgra.abund %>% filter(estuary == "nre")
pfgra.lqre <- pfgra.abund %>% filter(estuary == "lqre")
...
## ERE
pfg2.ere <- pfg.abund.ere %>%  
  ggplot(aes(x=disturbance, y=veg.fngp2.mean, color = fngp2, shape = endemic)) +   
  geom_jitter(position = pd.pfg2, 
             size = 7) +
  geom_errorbar(aes(ymin = ifelse(veg.fngp2.mean - veg.fngp2.se <0,0, veg.fngp2.mean - veg.fngp2.se), 
                    ymax = veg.fngp2.mean + veg.fngp2.se),
                width = .3, 
                position = pd.pfg2) + 
  labs(x = "", y = "Mean Relative Abundance \nAbove-Ground Cover", color = "Plant Functional Group", shape = "Endemism") +
  theme_classic()+
   theme(axis.text.y = element_text(colour = "black", size = 14, face = "bold"), 
    axis.text.x = element_text(colour = "black", size = 14, angle = 25, hjust = 1), 
    legend.text = element_text(size = 14, colour ="black", lineheight = 1), legend.key.height = unit(1.15, "cm"), 
  ...
  scale_x_discrete(limits = c("grubbed", "reference"), labels = c("Grubbed", "Undisturbed")) +
  scale_y_continuous(limits=c(0,1), breaks=c(0, 0.25, 0.5, 0.75, 1)) +
  scale_shape_discrete(limits = c("Native", "Invasive"), labels = c("Native", "Invasive")) +
  scale_color_manual(values = pfg2col)
pfg2.ere
...
#################################
## NMDS based on endemic.fngp2 abundance
#################################

# matrix of species presence/absence

ra.com.ere = ra.ere[,10:ncol(ra.ere)]
ra.com.nre = ra.nre[,10:ncol(ra.nre)]
ra.com.lqre = ra.lqre[,10:ncol(ra.lqre)]
...
#add columns to data frame 
ds.seedvegra.ere$id = ra.join.ere$id
ds.seedvegra.nre$id = ra.join.nre$id
ds.seedvegra.lqre$id = ra.join.lqre$id


ds.seedvegra.ere$disturbance = ra.join.ere$disturbance
ds.seedvegra.nre$disturbance = ra.join.nre$disturbance
ds.seedvegra.lqre$disturbance = ra.join.lqre$disturbance


ds.seedvegra.ere$obs = ra.join.ere$obs
ds.seedvegra.nre$obs = ra.join.nre$obs
ds.seedvegra.lqre$obs = ra.join.lqre$obs


ds.seedvegra.ere$estuary = ra.join.ere$estuary
ds.seedvegra.nre$estuary = ra.join.nre$estuary
ds.seedvegra.lqre$estuary = ra.join.lqre$estuary
...
seedvegra.nmds.ere <- ds.seedvegra.ere %>% 
    ...
    geom_jitter(position = pd.nmds.ra, size = 6)+ 
    #xlim(-1,1.5) + ylim(-1,1) +
    theme(axis.text.y = element_text(colour = "black", size = 20, face = "bold"), 
    axis.text.x = element_text(colour = "black", face = "bold", size = 20), 
    legend.text = element_text(size = 20, face ="bold", colour ="black", lineheight = 0.8), legend.key.height = unit(1, "cm"), 
    ...
    panel.background = element_blank(), panel.border = element_rect(colour = "black", fill = NA, size = 1.2),
    legend.key=element_blank()) + 
    labs(x = "NMDS1", colour = "Disturbance", shape = "Life Stage", y = "NMDS2", title = "Englishman", caption = "stress = 8.9e-05")  + 
    theme(strip.text.x = element_text(size = 20, face = "bold"), plot.caption = element_text(size = 20)) +
    scale_color_manual(limits = c("grubbed", "excl1", "excl10", "reference"), labels = c("Grubbed", "Heavily Grazed, \nExclosed 1 year", "Heavily Grazed, \nExclosed 10 years", "Undisturbed"), values = c("#762A83", "#C2A5CF", "#A6DBA0", "#1B7837")) +
    scale_shape_discrete(limits = c("veg", "seed"), labels = c("Vegetation", "Summer Seed Bank"))
seedvegra.nmds.ere
...
seedvegra.nmds.nre = ggplot(ds.seedvegra.nre, aes(x = NMDS1, y = NMDS2)) + 
    ...
    panel.background = element_blank(), panel.border = element_rect(colour = "black", fill = NA, size = 1.2),
    legend.key=element_blank()) + 
    labs(x = "NMDS1", colour = "Disturbance", shape = "Life Stage", y = "NMDS2", title = "Nanaimo", caption = "stress = 8.3e-05")  + 
    theme(strip.text.x = element_text(size = 20, face = "bold"), plot.caption = element_text(size = 20)) +
    scale_color_manual(limits = c("grubbed", "excl1", "excl10", "reference"), labels = c("Grubbed", "Heavily Grazed, \nExclosed 1 year", "Heavily Grazed, \nExclosed 10 years", "Undisturbed"), values = c("#762A83", "#C2A5CF", "#A6DBA0", "#1B7837")) +
    scale_shape_discrete(limits = c("veg", "seed"), labels = c("Vegetation", "Summer Seed Bank"))
seedvegra.nmds.nre
...
seedvegra.nmds.lqre = ggplot(ds.seedvegra.lqre, aes(x = NMDS1, y = NMDS2)) + 
    ...
    panel.background = element_blank(), panel.border = element_rect(colour = "black", fill = NA, size = 1.2),
    legend.key=element_blank()) + 
    labs(x = "NMDS1", colour = "Disturbance", shape = "Life Stage", y = "NMDS2", title = "Little Qualicum", caption = "stress = 8.8e-05")  + 
    theme(strip.text.x = element_text(size = 20, face = "bold"), plot.caption = element_text(size = 20)) +
    scale_color_manual(limits = c("grubbed", "excl1", "excl10", "reference"), labels = c("Grubbed", "Heavily Grazed, \nExclosed 1 year", "Heavily Grazed, \nExclosed 10 years", "Undisturbed"), values = c("#762A83", "#C2A5CF", "#A6DBA0", "#1B7837")) +
    scale_shape_discrete(limits = c("veg", "seed"), labels = c("Vegetation", "Summer Seed Bank"))
seedvegra.nmds.lqre
