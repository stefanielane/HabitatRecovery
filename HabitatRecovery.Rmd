...
```{r table of relabund by disturbance}
freq_df <- ra_join
...
```{r boxplots - veg richness & abundance: native vs exotic}
veg.sum <- mie %>% 
  filter(!species == "bare") %>% 
  filter(cover != 0) %>% 
...
```{r boxplots of native vs exotic tall, perennial graminoids in veg vs seed}
...
```{r NMDS of relative abundance just veg}
...
```{r NMDS of relative abundance seed vs veg}
...
# extract species scores 
#Using the scores function from vegan to extract the species scores and convert to a data.frame
sp.scores.nre <- as.data.frame(scores(nmds.compare.ra.lqre, "species"))  
