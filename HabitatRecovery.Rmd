...
ag_model <- ggplot() +
 ...
 # ggtitle("Actual vs. predicted proportion of tall, perennial graminoid 
...
bg_model <- ggplot() +
  ...
  scale_color_discrete(name = "Estuary", labels = c("Little Qualicum", "Nanaimo")) #+
  #ggtitle("Actual vs. predicted proportion of tall, perennial graminoid 
          #below-ground vegetation cover based on disturbance condition")
...
indic_sp_ag <- ra_join %>% 
              ...
              filter(species %in% c("agsp", "elpar", "coco", "juba", "caly", "trma", "spca", "glma", "popa", "sade", "juar"), obs == "veg") %>%               group_by(id) %>%             
              pivot_wider(names_from = species, values_from = relabund) %>% 
              replace(is.na(.), 0) %>% 
              mutate(juar = 0) %>% 
...
#ggsave("fig-box-agbg-richness.jpg", rich_box, width = 8, height = 6)
