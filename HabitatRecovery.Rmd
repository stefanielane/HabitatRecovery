...
ra.join <- mie.ra %>% 
  full_join(seed.ra, by = c("estuary", "disturbance", "id","species","obs","endemic.fngp2", "endemic","fngp", "fngp2","rel.abund", "season")) %>% 
...
pa.join <- pa.join[which(rowSums(pa.join[,7:29]) > 0), ] # remove rows where all species columns = 0
#View(pa.join)
#ncol(pa.join) #29
#colnames(pa.join)
# which(is.na(pa.join))
...
Vegetation trends
```{r vegetation trends - means & SE by relabund}
...
```{r veg NMDS by cover, single plot & facet by estuary}
...
```{r veg NMDS by cover, separate plots by estuary}
...
``` {r surface seed bank trends - means & SE by relabund}
...
#extract NMDS scores (x and y coordinates)
############################################
ds.seasonal.seed = as.data.frame(scores(nmds.seasonal.seed))
...
## all
ds.seasonal.seed$estuary = factor(ds.seasonal.seed$estuary, levels = c("ere", "nre", "lqre"))
...
seasonal.seed.nmds = ggplot(ds.seasonal.seed, aes(x = NMDS1, y = NMDS2)) + 
    ...
    legend.key=element_blank()) + 
    facet_grid(. ~ estuary, labeller = labeller(estuary = facet_labels)) +
    labs(x = "NMDS1", colour = "Disturbance", shape = "Surface seed bank \nsampling season", y = "NMDS2", caption = "Relative compositional abundance of seasonal surface seed bank samples (Bray's distance), stress = 0.132")  + 
    theme(strip.text.x = element_text(size = 20, face = "bold"), plot.caption = element_text(size = 20)) +
    scale_colour_brewer(palette = "PRGn", limits = c("grubbed", "excl1", "excl10", "reference"), labels = c("Grubbed", "Heavily Grazed, \nExclosed 1 year", "Heavily Grazed, \nExclosed 10 years", "Undisturbed")) +
...
```{r relabund PERMANOVA & NMDS all seed vs. veg}
...
compare.ra <- ra.join %>% 
  ungroup() %>% 
  filter(!obs == "bare") %>% 
  select(-sum.seed, -sum.veg, -endemic, -fngp2, -endemic.fngp2) %>% 
  pivot_wider(names_from = species, values_from = rel.abund) %>%
  replace(is.na(.), 0) 
#View(compare.ra)

# PERMANOVA
sp.compare.ra = compare.ra[,9:ncol(compare.ra)]
...
set.seed(42)
permanova.compare.ra <- adonis2(comp.ra.matrix ~ disturbance + obs + estuary + season, data = compare.ra, permutations = 99, method = "bray")
...
#View(compare.ra.ere)
#nrow(compare.ra.ere) #63
#ncol(compare.ra.ere) #33
#compare.ra.ere <- compare.ra.ere[which(rowSums(compare.ra.ere[,6:33]) > 0), ]


compare.ra.nre <- compare.ra %>% filter(estuary == "nre") 
#View(compare.ra.nre)
#nrow(compare.ra.nre) #96
#ncol(compare.ra.nre) #33

compare.ra.lqre <- compare.ra %>% filter(estuary == "lqre")
#View(compare.ra.lqre)
#nrow(compare.ra.lqre) #96
#ncol(compare.ra.lqre) #33

# matrix of species relative abundance

ra.compare.ere = compare.ra.ere[,6:ncol(compare.ra.ere)]
ra.compare.nre = compare.ra.nre[,6:ncol(compare.ra.nre)]
ra.compare.lqre = compare.ra.lqre[,6:ncol(compare.ra.lqre)]
#View(compare.ra.nre)

#turn abundance data frame into a matrix
mat.ra.ere = as.matrix(ra.com.ere)
# nrow(m.ra.ere) #63
# ncol(m.ra.ere) #28
# View(m.ra.ere)
...
ds.compare.ra.ere = as.data.frame(scores(nmds.compare.ra.ere))
ds.compare.ra.nre = as.data.frame(scores(nmds.compare.ra.nre))
ds.compare.ra.lqre = as.data.frame(scores(nmds.compare.ra.lqre))
...
seasoncol = c("#FC8D62", "#E78AC3", "#66C2A5")
brewer.pal(n = 4, "Set2")

## ERE
ds.compare.ra.ere$disturbance <- as.factor(ds.compare.ra.ere$disturbance)
...
compare.ra.nmds.ere <- ds.compare.ra.ere %>% 
    ...
    labs(x = "NMDS1", colour = "Season", shape = "Life Stage", y = "NMDS2", title = "Englishman", caption = "stress = ")  + 
    theme(strip.text.x = element_text(size = 20, face = "bold"), plot.caption = element_text(size = 20)) +
...
## NRE
ds.compare.ra.nre$disturbance <- as.factor(ds.compare.ra.nre$disturbance)
...
compare.ra.nmds.nre = ggplot(ds.compare.ra.nre, aes(x = NMDS1, y = NMDS2)) + 
    ...
    panel.background = element_blank(), panel.border = element_rect(colour = "black", fill = NA, size = 1.2),
    legend.key=element_blank()) + 
    labs(x = "NMDS1", colour = "Disturbance", shape = "Life Stage", y = "NMDS2", title = "Nanaimo", caption = "stress = ")  + 
    theme(strip.text.x = element_text(size = 20, face = "bold"), plot.caption = element_text(size = 20)) +
    scale_color_manual(limits = c("grubbed", "excl1", "excl10", "reference"), labels = c("Grubbed", "Heavily Grazed, \nExclosed 1 year", "Heavily Grazed, \nExclosed 10 years", "Undisturbed"), values = c("#762A83", "#C2A5CF", "#A6DBA0", "#1B7837")) +
...
## LQRE
ds.compare.ra.lqre$disturbance <- as.factor(ds.compare.ra.lqre$disturbance)
...
compare.ra.nmds.lqre = ggplot(ds.compare.ra.lqre, aes(x = NMDS1, y = NMDS2)) + 
    ...
    panel.background = element_blank(), panel.border = element_rect(colour = "black", fill = NA, size = 1.2),
    legend.key=element_blank()) + 
    labs(x = "NMDS1", colour = "Disturbance", shape = "Life Stage", y = "NMDS2", title = "Little Qualicum", caption = "stress = ")  + 
    theme(strip.text.x = element_text(size = 20, face = "bold"), plot.caption = element_text(size = 20)) +
    scale_color_manual(limits = c("grubbed", "excl1", "excl10", "reference"), labels = c("Grubbed", "Heavily Grazed, \nExclosed 1 year", "Heavily Grazed, \nExclosed 10 years", "Undisturbed"), values = c("#762A83", "#C2A5CF", "#A6DBA0", "#1B7837")) +
  scale_shape_manual(values = c(15, 17), limits = c("veg", "seed"), labels = c("Vegetation", "Seed Bank"))
...
## panel format w/ ggpubr
compare.ra.nmds.panel <- ggarrange(compare.ra.nmds.ere, compare.ra.nmds.nre, compare.ra.nmds.lqre,
          common.legend = TRUE, legend = "bottom",
          ncol = 3, nrow = 1) %>% 
...
#ggsave("fig-nmds-compare-ra-panel.jpg", seedvegra.nmds.panel, width = 20, height = 6)
