...
rich_box <- agbg_box %>%
  ...
  labs(x = "", y = "Total Species Richness", fill = "Endemism") +
...
ag_pred_mean <- predict(graminoid_glm, type = "response") 
veg.sum$gram_prop_mean = ag_pred_mean
#View(veg.sum)


ag_pred_se <- predict(graminoid_glm, type = c("response"), se.fit = TRUE)$se.fit
...
ag_model <- ggplot() +
  ...
  labs(x = "", 
       y = "Proportion of tall, perennial graminoids") 
...
bg_model <- ggplot() +
  geom_jitter(data = seed.sum, 
              aes(x = level_order, 
                  y = graminoid_prop,
                  color = estuary),
              size = 4) + 
    geom_errorbar(data = seed.sum,
                aes(x = level_order, 
...
agbg_pred_se <- predict(all_glm, type = c("response"), se.fit = TRUE)$se.fit
...
model.panel <- ggarrange(ag_model, bg_model,
                          labels = c("Above-ground vegetation", "Surface seed bank"),
...
```{r FIGURE - grouped barchart AG/BG relabund}


#unique(ra_join$species)
