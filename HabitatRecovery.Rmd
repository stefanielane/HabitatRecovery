...
library(car)
model <- lm(rel.abund ~ disturbance + estuary + obs + season + structure, data = ra.model)
Anova(model)
summary(model)
...
ggplot(fit$model, aes_string(x = names(fit$model)[2], y = names(fit$model)[1])) +
  geom_point() +
  #geom_pointrange(data = pred.lm , aes(x = x, y = fit, ymin = lwr, ymax = upr), size = 0.8, colour = "black") +
...
mod <- lme4::lmer(rel.abund ~ disturbance + estuary + obs + season + structure + (1|id), REML= FALSE, data= ra.model)
...
disturb_plot <- ggplot() + 
  geom_point(data=ra.model, aes(disturbance, rel.abund)) + 
